...
async function hasOnlyValidProperties(req, res, next) {
  const VALID_PROPERTIES = ["question", "answer_a", "answer_b","answer_c","answer_d","correct"];
  const { data = {} } = req.body;
...
async function hasRequiredProperties(req,res,next){
  const requiredProperties = ["question", "answer_a", "answer_b","answer_c","answer_d","correct"]
  const {data={}}= req.body;
...
async function list(req,res){
  try{
    ...
    console.error('An error occurred while retrieving the list of questions:', error);
